...
router.route(['/establishment/:id'])
    .get(function (req, res) {
        ...
        connection.query(`select u.username, u.id as id_user, com.name as "comment_name", com.comment, com.id as comment_id, com.status, etab.name as "establishment_name", etab.id_etablishment as id_etabl FROM establishment etab inner join etablishment_comments ac on etab.id_etablishment=ac.etablishment_id INNER JOIN comments com on ac.comment_id=com.id INNER JOIN users u ON ac.user_id=u.id WHERE ac.etablishment_id = ? AND com.status = 0`, id, (err, results) => {
            if (err) {
